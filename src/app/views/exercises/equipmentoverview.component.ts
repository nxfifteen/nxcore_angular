...
@Component({
  templateUrl: './equipmentoverview.component.html'
})
export class EquipmentOverviewComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.currentUser.firstrun) {
      ...
      this._matomoService.setupTracking('Core | Exercise Equipment Overview');
  ...
  buildViewContent(bustCache?: boolean): void {
    ...
    this.apiService.getExercisesEquipmentOverview(bustCache).subscribe((data) => {
